...
@Composable
fun MyApp(qrCodeViewModel: QrCodeViewModel = viewModel()) {
    val navController = rememberNavController()
    NavHost(navController = navController, startDestination = "splash") {
        composable("splash") { SplashScreen(navController) }
        composable("qrCode") { QrCodeScreen(navController, qrCodeViewModel) }
    ...
    LaunchedEffect(qrCodeViewModel.isFirebaseInitialized) {
        if (qrCodeViewModel.isFirebaseInitialized) {
